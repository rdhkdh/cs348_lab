...
// removes all whitespaces within string
...
//formats a number to be preceded by 'width' number of 0s. 0 can be replaced with any alternate character.
string format_number(int num, int width, char c = '0')
{
	stringstream temp;
	temp<<hex<<uppercase<<setfill(c)<<setw(width)<<num;
	return temp.str();
}

//formats a string to be padded with reqd no of whitespace from left
string format_string(string str, int width, char c = ' ')
{
	stringstream temp;
	temp<<left<<setfill(c)<<setw(width)<<str;
	return temp.str();
}
